...
export function AuthProvider({ children }) {
  ...
  useEffect(() => onAuthStateChanged(auth, async user => { setCurrentUser(user); if (!user) { setProfile(null); setLoading(false); return; } try { const result = await api('/me', user); setProfile(makeProfile(result.employee, user)); } catch { setProfile(null); } finally { setLoading(false); } }), []);
